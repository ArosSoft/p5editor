...
async function submitSketch() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Перенаправление в «Мои программы», чтобы пользователь мог видеть статус своего скетча
      addPublishStep('Завершение', 'Перенаправление в «Мои программы» через 3 секунды...', 'success')
      setTimeout(() => {
