...
async function submitSketch() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Перенаправление в личный кабинет, чтобы пользователь мог видеть статус своего скетча
      addPublishStep('Завершение', 'Перенаправление в личный кабинет через 3 секунды...', 'success')
      setTimeout(() => {
