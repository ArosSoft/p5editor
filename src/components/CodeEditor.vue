...
function selectMatch(index: number) {
  ...
  const wrapIndex =
    ((index % allMatches.value.length) + allMatches.value.length) %
...
const extensions = computed(() => {
  ...
  return [
    ...
    isDark
      ? EditorView.theme({
          '&': { backgroundColor: '#1a1b1f !important' },
          '.cm-scroller': { backgroundColor: 'transparent !important' },
          '.cm-gutters': {
            backgroundColor: '#15161b !important',
            color: '#4a4d55',
          },
        })
      : EditorView.theme(
    ...
    >
      ...
      <transition name="slide">
        <div v-if="showReplaceSection" class="replace-row">
          ...
          />
          <button @click="doReplaceNext" class="replace-btn">Заменить</button>
